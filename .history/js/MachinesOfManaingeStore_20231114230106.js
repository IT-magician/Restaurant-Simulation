...
class OrderManager {
  ...
  #update(orderNumber, statusName) {
    console.log(
      `update 함수 = 수정후 (인자값 : ${orderNumber}, ${statusName}) : `,
      this.#orderList,
    ...
    if (statusName === "요리중") alert(statusName);
    ...
    this.#displayDrawerFunc(this.#orderList, statusName);
  }
  ...
  append(food) {
    ...
    const waitingCookerArrival = async function (staff) {
      let result = null;
      const maxWatchTime = 1000 * 3600; // 1hour
      let watchTime = 0;
      let interval = 100;
    ...
    new Promise(async (resolve, reject) => {
      let cooker = await waitingCookerArrival(this.#waitingCookers);
