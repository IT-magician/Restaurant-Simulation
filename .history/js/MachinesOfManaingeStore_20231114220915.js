...
class OrderManager {
  ...
  #update(uuid, orderInfo, progressStatusName) {
    let deepCopied = [...this.#orderList];
    let arr = [];

    console.log(this.#orderList.find((item) => item.orderNumber === uuid));

    if (!this.#orderList.find((item) => item.orderNumber === uuid)) {
      orderInfo.statusName = progressStatusName + "";
      this.#orderList.push(orderInfo);
    }

    for (let orderInfo of deepCopied) {
      console.log(orderInfo);

      if (orderInfo.orderNumber === uuid) {
        console.log(uuid, progressStatusName);
        orderInfo.statusName = "A";
      } else arr.push(orderInfo);
    }
