class Staff {
    ...
    constructor(progress_status_info){
        if (progress_status_info instanceof Object === false || progress_status_info.length < 1)
            throw new Error("Invalid Argument - the progress status names's type must be object (key-value array) type and the length is more than zero.");
        ...
        let i = 0;
        for (const [progress_name, progress_waiting_time] of progress_status_info.entries()) {
    ...
    jumpCurrentProgress(){
        return this.#progress_status_idx = ++this.#progress_status_idx % this.#progress_status_names;
    }
    ...
    async doCurrentProgress(){
        const progressLevel = this.getCurrentProgressLevel();
        const progressWaitingTime = this.#progress_status_waiting_times[this.#progress_status_idx];

        return new Promise(function (resolve, reject) {
            setTimeout(()=>{
                jumpCurrentProgress();

                resolve(progressLevel);
            }, progressWaitingTime);
        });
        ...
        const countDown = function(){
            const leftWaitingTime = Math.max(0, getCurrentProgressWaitingTime() - progressStatusInterval);
    ...
    setProgressWaitingTime(progress_level, waiting_time) {
        if (progress_level < 1 || progress_level >= this.#progress_status_names.length)
