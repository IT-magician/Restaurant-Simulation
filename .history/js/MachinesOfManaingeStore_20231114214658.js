...
class OrderManager {
  ...
  async #update(uuid, progressStatusName) {
    ...
    if (targetIndex > -1) {
      const tmp = this.#orderList[targetIndex];
      this.#orderList.slice(targetIndex, 1);
      tmp.statusName = progressStatusName + " " + uuid + " " + targetIndex;
    }
  ...
  /*virtual */ async append(food) {
    ...
    this.#orderList.push(
      JSON.parse(JSON.stringify(new Data(food, uuid, null)))
    );
